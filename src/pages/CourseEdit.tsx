...
import {
  ...
  SlidersHorizontal, Globe, Lock, CreditCard, Link as LinkIcon
} from 'lucide-react';
...
export default function CourseEdit() {
  ...
  useEffect(() => {
    if (!loading && (!user || (!user.roles.includes('seller') && !user.is_admin))) {
      navigate('/login');
      return;
    }
    if (user && courseId) {
  ...
  const paymentLink = courseId ? `${window.location.origin}/pay/${courseId}` : '';
              ...
              <section className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-5">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="relative">
                      <input
                        ...
                        placeholder="0"
                        className="w-full pl-3.5 pr-10 py-2.5 text-sm border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-transparent transition-all"
                      />
                  ...
                  <div className="flex items-center justify-between py-1">
                    <div>
                      <div className="text-sm font-medium text-gray-900 dark:text-gray-100">Включить оплату</div>
                      <div className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Показывать страницу оплаты</div>
                    </div>
                    <button
                      onClick={() => setPaymentEnabled(p => !p)}
                      className={`relative flex-shrink-0 rounded-full transition-colors focus:outline-none ${paymentEnabled ? 'bg-teal-500' : 'bg-gray-200 dark:bg-gray-700'}`}
                      style={{ width: 40, height: 22 }}
                    >
                      <span className={`absolute top-0.5 left-0.5 w-[18px] h-[18px] bg-white rounded-full shadow-sm transition-transform ${paymentEnabled ? 'translate-x-[18px]' : 'translate-x-0'}`} />
                    </button>
                  </div>

                  {paymentEnabled && price && parseFloat(price) > 0 && (
                    <div className="pt-2 border-t border-gray-100 dark:border-gray-700">
                      <p className="text-xs text-gray-500 dark:text-gray-400 mb-2">Ссылка на страницу оплаты</p>
                      <div className="flex items-center gap-2">
                        <div className="flex-1 min-w-0 px-3 py-2 bg-gray-50 dark:bg-gray-900 text-gray-700 dark:text-gray-300 text-xs rounded-lg font-mono truncate border border-gray-200 dark:border-gray-700">
                          {paymentLink}
                        </div>
                        <button
                        ...
                        >
                          {copiedPayLink ? <Check className="w-4 h-4 text-teal-500" /> : <Copy className="w-4 h-4 text-gray-500 dark:text-gray-400" />}
                        </button>
                      </div>
                      <p className="text-xs text-teal-600 dark:text-teal-400 mt-2 flex items-center gap-1">
                        <LinkIcon className="w-3 h-3" />
                        Комиссия платформы 10% от каждой продажи
                      </p>
                    </div>
                  )}
                </div>
