...
class ApiClient {
  ...
  async createPost(data: { course_id: string; message_text?: string; text_content?: string }): Promise<CoursePost> {
    return this.post('/api/posts', data);
  }

  async updatePost(id: string, data: { message_text?: string; text_content?: string }): Promise<CoursePost> {
  ...
  async getAllFeaturedCourses(): Promise<any[]> {
    return this.get('/api/featured?all=true');
  }

  async getCoursePremiumStatus(courseId: string): Promise<{ premium_active: boolean; premium_expires_at: string | null }> {
    return this.get(`/api/courses/${courseId}/seller-premium`);
  }
