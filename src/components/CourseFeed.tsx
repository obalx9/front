...
function adaptApiPost(apiPost: ApiCoursePost & { media?: any[] }): CoursePost {
  const mediaItems = apiPost.media && apiPost.media.length > 0
    ? apiPost.media.map((m: any) => ({
        id: m.id,
        post_id: apiPost.id,
        media_type: m.media_type,
        storage_path: m.s3_url || null,
        s3_url: m.s3_url || null,
        telegram_file_id: m.telegram_file_id || null,
        telegram_thumbnail_file_id: m.thumbnail_s3_url || null,
  ...
  return {
    ...
    title: '',
    text_content: (apiPost as any).message_text || '',
    media_type: firstMedia?.media_type || null,
    storage_path: firstMedia?.storage_path || null,
    file_name: firstMedia?.file_name || null,
    file_size: firstMedia?.file_size || null,
    telegram_file_id: firstMedia?.telegram_file_id || null,
    telegram_thumbnail_file_id: firstMedia?.telegram_thumbnail_file_id || null,
    telegram_media_width: null,
    telegram_media_height: null,
    has_error: false,
    error_message: null,
    published_at: apiPost.created_at,
    ...
    try {
      if (editingPostId) {
        ...
        await api.patch(`/api/posts/${editingPostId}`, {
          message_text: formData.text_content,
          has_error: false,
          error_message: null,
        });
        ...
        if (newPostMediaFiles.length > 0) {
          await api.addPostMedia(newPost.id, newPostMediaFiles.map((file, index) => ({
            ...
            s3_url: api.getMediaPublicUrl(file.storage_path),
