...
import { Search, Video, Calendar, Pencil as Edit2, Trash2, X, Save, Upload, Plus, AlertTriangle, ArrowUp, ArrowDown, Pin, Megaphone, ExternalLink } from 'lucide-react';
...
export default function CourseFeed({
  ...
  const loadAdsAndPremium = async () => {
    try {
      const premiumStatus = await api.getCoursePremiumStatus(courseId).catch(() => null);
      const premiumValid = premiumStatus?.premium_active
        && premiumStatus?.premium_expires_at
        && new Date(premiumStatus.premium_expires_at) > new Date();
