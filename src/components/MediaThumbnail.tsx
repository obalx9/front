...
export default function MediaThumbnail({
  ...
  useEffect(() => {
    const fetchThumbnail = async () => {
      try {
        ...
        const isVidType = mediaType === 'video' || mediaType === 'animation' || !!(fileName && /\.(mp4|mov|avi|mkv|webm|m4v|3gp|flv|wmv)$/i.test(fileName));

        // For videos/animations without thumbnail_file_id, skip loading
        if (isVidType && !thumbnailFileId) {
          setLoading(false);
          return;
        }
        ...
        const fileIdToLoad = (isVidType && thumbnailFileId) ? thumbnailFileId : fileId;
  ...
  const isVideoByExt = !!(fileName && VIDEO_EXT_RE.test(fileName));
  const isVideoType = mediaType === 'video' || mediaType === 'animation' || isVideoByExt;
  ...
  if (isActive && isVideoType && courseId && postId && onCloseInline) {
    return (
      <div className="w-full max-w-2xl animate-scale-in">
        <VideoPlayer
          mediaUrl={getSecureMediaUrl(fileId)}
          mediaType="video"
          fileName={fileName}
  ...
  if (loading) {
    const loadingHeight = actualWidth && actualHeight
      ? Math.min(Math.round(maxWidth / (actualWidth / actualHeight)), 500)
      : 300;
    return (
      <div
        className="bg-gray-200 rounded-lg flex items-center justify-center cursor-pointer"
        style={{ width: '100%', minHeight: loadingHeight }}
      >
  ...
  if (error || !thumbnailUrl) {
    const fallbackHeight = actualWidth && actualHeight
      ? Math.min(Math.round(maxWidth / (actualWidth / actualHeight)), 500)
      : 300;
    return (
      <div
        ...
        style={{ width: '100%', minHeight: fallbackHeight }}
        onClick={handleClick}
      >
        {isVideoType ? (
          <>
