...
export default function MediaGroupEditor({
  ...
  const migrateLegacyMedia = async () => {
    ...
    try {
      ...
      for (const item of legacyItems) {
        ...
        const newMedia = await api.post(`/api/posts/${postId}/media`, {
          media_type: item.media_type,
          storage_path: item.storage_path || null,
          telegram_file_id: item.telegram_file_id,
          telegram_thumbnail_file_id: item.telegram_thumbnail_file_id || item.telegram_thumbnail || null,
  ...
  const handleDeleteMedia = async (mediaId: string) => {
    ...
    try {
      ...
      await api.delete(`/api/posts/media/${mediaId}`);
    ...
    try {
      ...
      await Promise.all(
        items.map(async (item, index) => {
          await api.patch(`/api/posts/media/${item.id}`, { order_index: index });
        })
      );
      ...
      for (const item of mediaItems) {
        ...
        try {
          ...
          const isVideo = item.media_type === 'video' || item.media_type === 'animation' ||
            !!(item.file_name && /\.(mp4|webm|mov|avi|mkv|m4v|3gp|flv|wmv)$/i.test(item.file_name));

          if (isVideo && item.telegram_thumbnail_file_id) {
            url = api.getMediaUrl(`telegram/${item.telegram_thumbnail_file_id}/${courseId}`);
          } else if (item.storage_path) {
            if (isVideo) {
              setImageUrls(prev => new Map(prev).set(item.id, '__video_placeholder__'));
              continue;
            }
            ...
            url = api.getMediaUrl(`telegram/${item.telegram_file_id}/${courseId}`);
          } else {
            continue;
          }
