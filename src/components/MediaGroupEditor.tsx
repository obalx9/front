...
export default function MediaGroupEditor({
    ...
    try {
      ...
      for (const item of legacyItems) {
        ...
        const newMedia = await api.post(`/api/posts/${postId}/media`, {
          ...
          s3_url: item.s3_url || (item.storage_path ? api.getMediaPublicUrl(item.storage_path) : null),
          telegram_file_id: item.telegram_file_id,
          thumbnail_s3_url: item.telegram_thumbnail,
  ...
  const handleAddMedia = async (storagePath: string, fileSize: number, fileName: string) => {
    try {
      ...
      const newMedia = await api.post(`/api/posts/${postId}/media`, {
        media_type: mediaType,
        s3_url: api.getMediaPublicUrl(storagePath),
        file_name: fileName,
        file_size: fileSize,
        order_index: mediaItems.length
      });
      ...
      for (const item of mediaItems) {
        ...
        try {
          ...
          if (item.storage_path) {
            ...
            const data = await api.generateMediaToken(item.telegram_file_id, courseId);
            url = data.access_token ? api.getMediaUrl(`telegram/${item.telegram_file_id}/${courseId}`) : '';
          } else {
            continue;
          }
