...
export function getBackgroundOverlay(bg: string, opacity: number): BackgroundOverlayStyle {
  if (bg.includes('gradient')) {
    return {
...
export function getPostStyles(
  ...
  const overlay = getBackgroundOverlay(theme.bg, theme.bgOpacity);

  const containerBg = overlay.needsOverlay
    ? {}
    : overlay.containerStyle;
...
export function getPostBgOverlayStyle(
  theme: PostTheme,
  borderRadius: number
): CSSProperties | null {
  const overlay = getBackgroundOverlay(theme.bg, theme.bgOpacity);
  if (!overlay.needsOverlay) return null;
  return {
