...
export function ensureSafeThemeConfig(config: ThemeConfig | null | undefined): ThemeConfig {
  const defaults = themePresets[0].config;
  if (!config) return defaults;
  return {
    background: config.background || defaults.background,
    header: config.header || defaults.header,
    posts: {
      style: config.posts?.style || defaults.posts.style,
      borderRadius: config.posts?.borderRadius ?? defaults.posts.borderRadius,
      light: config.posts?.light || defaults.posts.light,
      dark: config.posts?.dark || defaults.posts.dark,
      emojiPattern: config.posts?.emojiPattern || defaults.posts.emojiPattern,
    },
    animations: config.animations || defaults.animations,
  };
}
